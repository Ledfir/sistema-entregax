...
const IngresosDiariosTdi: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    setLoading(true)
    try {
      const resp = await apiClient.get(`/cedis/ingresos-diarios/tdi-express/${fechaStr}`)
  ...
  const filtered = React.useMemo(() => {
    ...
    return data.filter((r) =>
      ...
      (r.quienrecibe || '').toLowerCase().includes(q)
    )
  }, [data, search])

  const columns: ColumnsType<any> = [
    { title: 'Cliente', dataIndex: 'suite', key: 'suite', align: 'center' },
    { title: 'Guia de ingreso', dataIndex: 'guiaingreso', key: 'guiaingreso', align: 'center' },
    { title: 'Guia unica', dataIndex: 'guiaunica', key: 'guiaunica', align: 'center' },
    { title: 'Quien ingreso', dataIndex: 'quienrecibe', key: 'quienrecibe', align: 'center' },
    {
  ...
  const handlePrint = () => {
    const title = `LISTADO DE GUIAS TDI INGRESADAS EL ${fecha.format('DD-MM-YYYY')}`
    const rows = filtered
      .map(
        (r) => `
      <tr>
        ...
        <td style="padding:8px">${r.quienrecibe || ''}</td>
    ...
    const html = `
      ...
      <body>
        ...
        <table>
          <thead>
            <tr><th>Suite</th><th>Guia de ingreso</th><th>Guia única</th><th>Quien recibe</th><th>Hora de entrada</th></tr>
          </thead>
          <tbody>${rows}</tbody>
        </table>
      </body>
  ...
  return (
    <div style={{ padding: 12 }}>
      <Card title="Ingresos diarios" bordered>
        ...
        <div style={{ marginTop: 16 }}>
          <Row justify="space-between" align="middle">
            <Col>
              <h3>LISTADO DE GUIAS TDI INGRESADAS EL {fecha.format('DD-MM-YYYY')}</h3>
...
export default IngresosDiariosTdi
