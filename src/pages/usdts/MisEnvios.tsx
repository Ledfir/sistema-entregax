...
export const MisEnvios = () => {
  ...
  const cargarEnvios = async () => {
    ...
    try {
      ...
      // const response = await operacionesService.getMyUSDTShipments();
      ...
      console.error('Error al cargar envíos USDT:', error);
      message.error('Error al cargar los envíos USDT');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleArchivar = async (record: Envio) => {
    ...
    if (result.isConfirmed) {
      try {
        // TODO: Implementar llamada al API
        // await operacionesService.archiveUSDTShipment(record.id);
