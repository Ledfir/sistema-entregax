...
export const MisEnvios = () => {
  ...
  const cargarEnvios = async () => {
    ...
    try {
      ...
      // const response = await operacionesService.getMyRMBShipments();
      ...
      console.error('Error al cargar envíos RMB:', error);
      message.error('Error al cargar los envíos RMB');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleBorrar = async (record: Envio) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        // await operacionesService.deleteRMBShipment(record.id);
  ...
  const handleArchivar = async (record: Envio) => {
    ...
    if (result.isConfirmed) {
      try {
        // TODO: Implementar llamada al API
        // await operacionesService.archiveRMBShipment(record.id);
  ...
  const columns: ColumnsType<Envio> = [
    ...
    },
    {
      ...
      render: (total: number) => `¥${total.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
    },
    {
      title: 'Estado',
      dataIndex: 'estado',
