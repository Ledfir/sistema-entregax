...
const Cancelar: React.FC = () => {
  ...
  const handleEnter = async () => {
    ...
    setLoading(true);
    try {
      const res = await apiClient.post('/cedis-usa/cancelar', { guia: guide });
      const data = res.data ?? {};
      const msgs: string[] = [];
      ...
      if (msgs.length) {
        ...
        success ? message.success('Guía cancelada correctamente') : message.error('Error en la petición');
      }
  ...
  return (
    <div style={{ padding: 24, display: 'flex', justifyContent: 'center' }}>
      <Card title="Cancelar" style={{ width: '100%', maxWidth: 700 }}>
        <div style={{ paddingTop: 12 }}>
          <label style={{ display: 'block', textAlign: 'center', marginBottom: 8, fontWeight: 600 }}>
            Escanea guia de salida
          </label>
          <Input
            ref={inputRef}
            placeholder="Escanea guia de salida"
            value={guia}
