...
export const Juego = () => {
  ...
  const playerRef = useRef<THREE.Group | undefined>(undefined);
  const mapRef = useRef<THREE.Group | undefined>(undefined);
  
  // Game state refs
  const metadataRef = useRef<any[]>([]);
  const positionRef = useRef({ currentRow: 0, currentTile: 0 });
  const movesQueueRef = useRef<string[]>([]);
  const moveClockRef = useRef<THREE.Clock | undefined>(undefined);
  ...
  const addRows = () => {
    ...
    newMetadata.forEach((rowData: any, index: number) => {
  ...
  const animatePlayer = () => {
    // No animar si el juego terminó
    if (isGameOverRef.current || !movesQueueRef.current.length || !moveClockRef.current) return;
    ...
    if (row.type === 'car' || row.type === 'truck') {
      ...
      row.vehicles.forEach(({ ref }: any) => {
        ...
        if (playerBoundingBox.intersectsBox(vehicleBoundingBox)) {
          ...
          movesQueueRef.current = []; // Limpiar movimientos pendientes
          if (moveClockRef.current && moveClockRef.current.running) {
            moveClockRef.current.stop();
          }
  ...
  const handleSaveScore = async () => {
    try {
      const response = await apiClient.post('/game/save-score', {
    ...
    <div style={{ position: 'relative', width: '100%', height: '100vh', overflow: 'hidden' }}>
      ...
      {showResult && (
        <div id="result-container">
          <div id="result">
            <h1>Fin del juego</h1>
            <p>Tu puntuación: <span>{finalScore}</span></p>
            <button onClick={initializeGame}>Reintentar</button>
            <br />
